...
public class DeviceTypesForm : AppForm
{
    ...
    private async Task AddAsync(){ using var f=new LookupEditForm("Thêm loại thiết bị",false); if(f.ShowDialog()!=DialogResult.OK)return; await using var db=new AppDbContext(); if(await db.DeviceTypes.IgnoreQueryFilters().AnyAsync(x=>x.Name==f.ItemName)){MessageBox.Show("Tên loại thiết bị đã tồn tại.");return;} db.DeviceTypes.Add(new Models.DeviceType{Name=f.ItemName}); await db.SaveChangesAsync(); await LoadDataAsync(); }
    private async Task EditAsync(){var id=Id();if(id is null)return;await using var db=new AppDbContext();var e=await db.DeviceTypes.FindAsync(id);if(e is null)return;using var f=new LookupEditForm("Sửa loại thiết bị",false,name:e.Name);if(f.ShowDialog()!=DialogResult.OK)return;if(await db.DeviceTypes.IgnoreQueryFilters().AnyAsync(x=>x.Name==f.ItemName&&x.Id!=id)){MessageBox.Show("Tên loại thiết bị đã tồn tại.");return;}e.Name=f.ItemName;await db.SaveChangesAsync();await LoadDataAsync();}
    private async Task DeleteAsync()
    {
        var id = Id();
        if (id is null || !Ui.ConfirmSoftDelete("loại thiết bị đã chọn")) return;
        await using var db = new AppDbContext();
        if (await db.Devices.AnyAsync(x => x.DeviceTypeId == id))
        {
            MessageBox.Show("Loại thiết bị đang được thiết bị hiện hành sử dụng nên chưa thể xóa.");
            return;
        }
